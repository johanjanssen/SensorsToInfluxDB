...
@Component
public class BME280RetrieveSensorData implements RetrieveSensorData {

	public BME280SensorData retrieve() {
		// Create I2C bus
		I2CBus bus;
		...
		try {
			...
			e.printStackTrace();
			throw new SensorException("BME280 sensor failed", this.getClass().getSimpleName());
		}
		...
		try {
			...
			dig_H1 = ((byte) device.read(0xA1) & 0xFF);
		...
		double cTempRounded = Rounding.round(cTemp, 1);
		double humidityRounded = Rounding.round(humidity, 1);
		double pressureRounded = Rounding.round(pressure, 1);
		
		return new BME280SensorData(cTempRounded, humidityRounded, pressureRounded);
	}
